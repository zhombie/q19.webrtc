  ...
  private void bluetoothTimeout() {
    ...
    if (devices.size() > 0) {
      ...
      if (bluetoothHeadset.isAudioConnected(bluetoothDevice)) {
        Log.d(TAG, "SCO connected with " + (bluetoothDevice != null ? bluetoothDevice.getName() : null));
        scoConnected = true;
      } else {
        Log.d(TAG, "SCO is not connected with " + (bluetoothDevice != null ? bluetoothDevice.getName() : null));
      }
    }
    if (scoConnected) {
